...
function darkMode() {
    chrome.storage.sync.get("dark", (items) => {
        ...
        if (items.dark === undefined) {
            ...
            style.setProperty("--moon", "#fff68f");
            style.setProperty("--moonBtn", "#6c7a89");
        }
        else {
            const style = document.documentElement.style
            if (items.dark === "true") {
                ...
                style.setProperty("--bodyBackground", "#ffffff");
                style.setProperty("--background", "#eeeeee");
                style.setProperty("--textColor", "#2D3135");
                style.setProperty("--baseColor", "#2e3131");
                style.setProperty("--moon", "#000000");
                style.setProperty("--moonBtn", "#dadfe1");

            } else if (items.dark === "false") {
                //dark mode
                chrome.storage.sync.set({ "dark": "true" }, function () { });
                style.setProperty("--bodyBackground", "#2e3131");
                style.setProperty("--background", "#2D3135");
...
function changeLang() {
    chrome.storage.sync.get("value", function (items) {
        const LS_VALUE = items.value;
        img = document.querySelector(".selectLangImg")
        if (LS_VALUE == "KR") {
...
function init() {
    ...
    document.querySelector(".selectLang").addEventListener("click", changeLang)
    chrome.storage.sync.get("value", function (items) {
        LS_VALUE = items.value;
        if (LS_VALUE === undefined) {
            chrome.storage.sync.set({ "value": "KR" }, function () {
                // 저장 완료
            });
            img = document.querySelector(".selectLangImg")
